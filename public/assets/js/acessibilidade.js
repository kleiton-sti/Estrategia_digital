...
(function () {
  ...
  function injetarBotaoVLibras() {
    ...
    btn.addEventListener('click', () => {
      // O botão real de ativação do VLibras fica dentro de um
      // Shadow DOM: #vlibras-access-wrapper > shadowRoot > <button>.
      // O id interno do botão varia entre versões do plugin, então
      // buscamos pela tag <button> em vez de depender de um id fixo.
      const wrapper = document.getElementById('vlibras-access-wrapper');
      const shadowBtn = wrapper && wrapper.shadowRoot
        ? wrapper.shadowRoot.querySelector('button')
        : null;
      ...
      // Fallback para versões antigas do plugin que usam [vw-access-button]
  ...
  /* — Força a ocultação do botão oficial do VLibras —
   * O vlibras-plugin.js aplica estilos inline diretamente no
   * elemento quando o widget é ativado ou reposicionado, o que
   * pode sobrescrever CSS externo. Cobrimos tanto o wrapper atual
   * (#vlibras-access-wrapper, com Shadow DOM) quanto o atributo
   * legado ([vw-access-button]) de versões antigas do plugin.
   */
  function forcarOcultacaoVLibras() {
    const wrapper = document.getElementById('vlibras-access-wrapper');
    if (wrapper) {
  ...
  forcarOcultacaoVLibras();
  window.addEventListener('load', forcarOcultacaoVLibras);

  const vlibrasObserver = new MutationObserver(forcarOcultacaoVLibras);
  vlibrasObserver.observe(document.body, {
    childList: true,
    subtree: true,
    attributes: true,
